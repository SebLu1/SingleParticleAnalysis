...
TAU_2_FUDGE = [100.0, 1000.0, 10000000.0]
...
if run_EM:
        ...
        for p in PDB_ID:
            for n in noise_level:
                if MPI_MODE == 'mpirun':
                    ...
                    raise Exception 
                refine_cmd += ' --o {OP}/EM_tau2fudge_{tau2f}/{p}/{p}_mult0{n}'
    ...
    #            refine_cmd += ' --preread_images' #  Use this to let the master process read all particles into memory.
                ...
                refine_cmd = refine_cmd.format(OP=out_path, p=p, n=n, GPU_ids=GPU_ids, NUM_MPI=NUM_MPI, tau2f=T2F)
