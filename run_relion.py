...
for p in PDB_ID:
    for n in noise_level:
        ...
        proj_cmd = mult_cmd.format(BP=base_path, p=p, n=n)
...
for p in PDB_ID:
    for n in noise_level:
        refine_cmd = 'relion_refine --o {BP}/Refine3D/{p}/{p}_mult0{n}'
        refine_cmd += ' --auto_refine --split_random_halves --i {BP}/projs/{p}_mult0{n}.star'
        refine_cmd += ' --ref {BP}/mult_maps/{p}_mult0{n}.mrc --ini_high 30'
        refine_cmd += ' --pad 1'
...
#        refine_cmd += '--pool' # Number of images to pool for each thread task
